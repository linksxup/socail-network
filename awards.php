...
<script>
  ...
  <nav id="nav">
   ...
   <li><button>  Top awarded &nbsp <font color='gold'><i class="fa fa-star"></i></font>&nbsp
   linkupdates</button></li>
  </ul>
...
if(isset($user)){
	//confirm if logged in user has made a choice of xup feed control
...
<div  align= 'center'><button id='alert'><b>Get started now</b></button></div>	
		...
		$query = "SELECT * FROM linkupdates  ORDER  BY awards DESC LIMIT $limit OFFSET 0;";
		$query1 = "SELECT * FROM linkupdates  ORDER  BY awards DESC";
