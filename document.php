...
{ 
 ...
 require'database.php';
	...
	$allowedimage= array('0','1st','600','602','abw','acl','afp','ami',
	'ans','asc','aww','ccf','csv','cwk','dbk','dita','doc','docm','docx','dot',
	'dotx','dwd','egt','epub','ezw','fdx','ftm','ftx','gdoc','html','hwp','hwpml',
	'hwpml','log','lwp','mbp','md','me','mcw','mobi','nb','nbp','neis','odm','odoc',
	'odt','osheet','ott','omm','pages','pap','pdax','pdf','quox','radix','rtf','rpt',
	'sdw','se','stw','sxw','tex','info','troff','txt','uof','uoml','via','wpd','wps',
	'wpt','wrd','wrf','wri','xhtml','xml','xps', 'ppt');
		...
		if($file_error[$j] == 0){
			if($file_size[$j] > 26104000){echo'<fieldset><h4>
<font color="red">document size too large. maximum allowable image size is 25mb</font></h4></fieldset>';
			}if($file_size[$j] < 26104000){
	$label='linksxup';

	$uniqueid = uniqid('', true);
	$filenewname = $label.uniqid('', true).'.'.$fileactualext;
	$filedestination= "https://storage.googleapis.com/documentzz/$filenewname";
	
	$filedestinationreal= "documentzz";
...
'$fileactualext1', '$uniqueid', '$row[firstname]', '$row[lastname]', 
'$row[profilepic]','$row[verification]', '$row[gender]','','0','0','','')");	
	 
	 
	 //create notifications
require'database.php';
					 $stm=mysqli_query($link, "INSERT INTO notifications (username, status, url, achieve, format, message,uniqueid) VALUES ('$user', 'regular', 'remarks.php?pid=$uniqueid', 'read', 'linkupdate/document', 'posted a linkupdate', '')");	
...
<h4 align= "center"><font color="57b846"> document linkupdate successful</font></h4>
...
<font color='red'>document type not supported</font></h4></fieldset>";}
	
		
		//if($file_error[$j]!=0){echo'<fieldset><h1>
//<font color="red">An error occured uploading the document</font></h1></fieldset>';}
...
<font color="red"> The maximum number of images to linkupdate is 1. </font></h4>
...
echo'<fieldset><h4 align= "center">
<font color="red"> Please select a document to linkupdate. </font></h4></fieldset>';
}
