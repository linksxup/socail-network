...
{ 
	...
	$allowedimage= array('3gp','aa','aac','aax','act','aiff','alac','amr','ape','au','awb','dct','dss','dvf','flac','gsm','iklax','ivs','m4a','m4b','m4p','mmf','mp3','mpc','msv','nmf','nsf','ogg','oga','mog','opus','ra','rm','raw','sln','tta','voc','vox','wav','wma','wv','webm','8svx', '3gpp');
	...
	//print_r($file_error);
		...
		if($file_error[$j] == 0){
			if($file_size[$j] > 12582912){echo'<fieldset><h4>
<font color="red">audio size too large. maximum allowable image size is 20mb</font></h4></fieldset>';
			}if($file_size[$j] < 12582912){
	$label='linksxup';

 $uniqueid = uniqid('', true);
	$filenewname = $label.uniqid('', true).'.'.$fileactualext;
	$filedestination= "https://storage.googleapis.com/audioz/$filenewname";
	
	$filedestinationreal= "audioz";
...
$stmt=mysqli_query($link, "INSERT INTO linkupdates (username, url, caption, file_type, uniqueid, firstname, lastname, profilepic,verification, gender,remarks,views,awards,awards1,likes) VALUES ('$user', '$filedestination', '$cap', '$fileactualext1', '$uniqueid', 
'$row[firstname]', '$row[lastname]', '$row[profilepic]','$row[verification]', 
'$row[gender]','','0','0','','')");	
	

//create notifications
require'database.php';
					 $stm=mysqli_query($link, "INSERT INTO notifications (username, status, url, achieve, format, message, uniqueid) VALUES ('$user', 'regular', 'remarks.php?pid=$uniqueid', 'read', 'linkupdate/audio', 'posted a linkupdate', '')");	
...
<font-color='red'>audio type not supported</font></h4></fieldset>";}
...
";
 ...
 <fieldset>
 <h4 align= "center"><font color="57b846"> audio linkupdate successful</font></h4>
...
<font color="red"> The maximum number of audio to linkupdate is 2. </font></h4>
...
echo'<fieldset><h4 align= "center">
<font color="red"> Please select an audio to linkupdate. </font></h4></fieldset>';
}
