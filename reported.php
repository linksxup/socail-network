...
<script>
  ...
  <nav id="nav">
   ...
   <li><button>Report submit response</button></li>
  </ul>
...
$cap=addslashes($_POST['textstatus']);
	$to=$_POST['un'];
$uniqueid=$_POST['uniqueid'];
if(!empty($cap && $to && $uniqueid)){
require'database.php';
$query = "SELECT * FROM linkupdates WHERE uniqueid='$uniqueid'";
$result = mysqli_query($link, $query);
$row=mysqli_fetch_array($result);
$file_type=$row['file_type'];
$url=$row['url'];

$stmt=mysqli_query($link, "INSERT INTO report (accuser, accused, issue, contentid, 
file_type, url) VALUES ('$user', '$to', '$cap', '$uniqueid', '$file_type', '$url')");
...
if($stmt){echo"<h4>Your report has been submitted successfully and our management team will look into your report... Thank you....</h4> &nbsp";
}else{
	echo"<h4>Operation failed</h4>";}

}else{echo"<h4>Please fill in the report field..</h4>";}
