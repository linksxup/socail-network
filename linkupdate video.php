...
?>
  ...
  <nav>
 <form action='linkupdate video.php' method='post'>
...
{ 
	...
	$allowedimage= array('avi','flv','wmv','mov','mp4','webm','mkv','vob','ogv','ogg','drc','gifv','mng','mpeg','svi','3gp','3g2','mxf','roq','nsv','amv','asf');
		...
		if($file_error[$j] == 0){
			if($file_size[$j] > 20971520){echo'<fieldset><h4>
<font color="red">video size too large. maximum allowable video size is 20mb</font></h4></fieldset>';
			}if($file_size[$j] < 20971520){
	$label='linksxup';

	$uniqueid = uniqid('', true);
	$filenewname = $label.uniqid('', true).'.'.$fileactualext;
	$filedestination= "https://storage.googleapis.com/videozz/$filenewname";
	
	$filedestinationreal= "videozz";
...
$stmt=mysqli_query($link, "INSERT INTO linkupdates (username, url, caption, 
file_type, uniqueid, firstname, lastname, profilepic,verification, gender,remarks,views,awards,awards1,likes) VALUES ('$user', '$filedestination', '$cap', '$fileactualext1', '$uniqueid', '$row[firstname]', '$row[lastname]', 
'$row[profilepic]','$row[verification]', '$row[gender]','','0','0','','')");	
	 
	 
//create notifications
require'database.php';
					 $stm=mysqli_query($link, "INSERT INTO notifications (username, status, url, achieve, format, message, uniqueid) VALUES ('$user', 'regular', 'remarks.php?pid=$uniqueid', 'read', 'linkupdate/video', 'posted a linkupdate', '')");	
...
<font-color='red'>video type not supported</font></h4></fieldset>";}
...
<h4 align= "center"><font color="57b846"> video linkupdate successful</font></h4>
...
<font color="red">Oops the maximum number of video to linkupdate at a time is 1. With a max file size of 25mb. </font></h4>
...
echo'<fieldset><h4 align= "center">
<font color="red"> Please select a video to linkupdate. </font></h4></fieldset>';
}
