...
//$ro=mysqli_fetch_array($bi);
		...
		header("location:info.php?un=$user");
		exit();
		
	}else{header('location: testt.php');
	exit();}
	}if($status == 'remark'){
		// this scenerio, the string $uniqueid is assigned to a secondryuniqueid from the remark db
		require'database.php';
		
		$bi=mysqli_query($link, "SELECT * FROM remarks WHERE secondaryuniqueid='$uniqueid'");	
$ro=mysqli_fetch_array($bi);
	if($ro){$b=mysqli_query($link, "SELECT * FROM linkupdates WHERE uniqueid='$ro[primaryuniqueid]'");	
	
$r=mysqli_fetch_array($b);
$u=unserialize($r['remarks']);
...
$querry = "UPDATE linkupdates SET remarks='$uii' WHERE uniqueid='$ro[primaryuniqueid]'";
		...
		$delete=mysqli_query($link, "DELETE FROM remarks WHERE secondaryuniqueid='$uniqueid'");
			...
			if($delete){
				...
				$del=mysqli_query($link, "DELETE FROM replies WHERE uniqueid='$uniqueid'");
	...
	header("location:remarks.php?pid=$ro[primaryuniqueid]");
	exit();
	}
	
	
	}
	//else{header('location: testt.php');exit();
	//}
...
$querry = "UPDATE remarks SET replies='$uiii' WHERE secondaryuniqueid='$uniqueid'";
			$res = mysqli_query($link, $querry);
			

		$time=$_GET['time'];
		
		$d=mysqli_query($link, "DELETE FROM replies WHERE  username='$user' AND time='$time' ");

		
	}
	
	if($d)
	{header("location:replies.php?sid=$uniqueid&pid=$c[primaryuniqueid]");
	exit();}
 }
  ...
  //if status is Moment
  if($status =='imagemoment'){
	  $id=$_GET['uniqueid'];
	  
	  require'database.php';

		$que = "SELECT * FROM moment2  WHERE uniqueid='$id'";
		$moment = mysqli_query($link, $que);
//while($mom= mysqli_fetch_array($moment)){
		//delete media credentials from moment2
		$mom= mysqli_fetch_array($moment);
	...
	//Delete from moment2
$stmt=mysqli_query($link, "DELETE FROM moment2 WHERE uniqueid='$mom[uniqueid]'");
