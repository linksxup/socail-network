...
<html>
    <head>
        <title>your Moments views</title>
        <meta charset="UTF-8">
...
$('b').css('color','#fff');
   ...
   echo"<button>";
	...
	$query = "SELECT * FROM moment2 WHERE uniqueid= '$uname'";
...
$row = mysqli_fetch_array($result);
	   ...
	   $u=$row['viewers'];
	   if($row['viewers'] != '')
	   {print_r(count(unserialize($u)));} echo"&nbsp viewer(s)";
   echo"</button>
   </form>";
...
$result= mysqli_fetch_array($next);
	...
	require'database.php';
		...
		$query = "SELECT * FROM moment2 WHERE uniqueid='$pid'";
...
/////////////////
 ...
 <li><b>'; echo'&nbsp views &nbsp<i class="fa fa-arrow-circle-o-down"></i></b></li>
</ul>
<hr></fieldset>';
	...
	require'database.php';
		...
		$query1 = "SELECT * FROM moment2  WHERE uniqueid='$pid'";
		$result1 = mysqli_query($link, $query1);
	
	$row = mysqli_fetch_array($result1);
	
		if(empty($row['viewers'])){echo"<fieldset>No user has currently viewed your moment</fieldset>";}
		
		
		else{
		$u=unserialize($row['viewers']);
